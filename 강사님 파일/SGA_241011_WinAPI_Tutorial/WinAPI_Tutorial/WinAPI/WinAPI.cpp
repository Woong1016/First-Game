...
//1. 다른건 몰라도 여기까지는 필수 
//-> 프레임 워크 찾아보기
// 이파일에 있는 Gamemanager 클래스와 Vector2 구조체를 찾아서
// 각각 어떤 내용이 있었는지 찾아보시고
// 선언을 따라 Tutorial Scene 까지 다시 찾아가 보도록 합시다.

//2. 실습
// 클릭과 사각형 만들기 (클릭 대신 키보드 다른 키여도 됩니다.)
// 클릭할 때마다 사각형이 윈도우 내에서 랜덤으로 만들어지게 해주세요

//3. 과제 + 예습

// rect 클래스의 충돌 함수를 참고해주세요
// 그리고 사각형 한개 이상을 한 윈도우에서 만든 후,
// 마우스 커서를 따라가는 사각형을 별도로 하나 더 만들어주세요
// 그리고 두 사각형이 부딪치면 지워지게 해주세요 .... 넹
//	(*사실 코드는 있습니다. 쓰시면서 그 원리를 이해해주세요)
// 왜냐하면 사각형 충돌의 원리를 이후에 다른 곳에서도 쓸 거거든요 ㅎㅎ
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    {
    case WM_CREATE:
        GameManager::Get()->Init(hWnd); /// 윈도우 생성시, "게임매니저"라는 것을 만든다.
        SetTimer(hWnd, 0, 10, nullptr);	/// 윈도우 기능 "타이머 세팅" 수행
										/// 정해진 초마다 (현재100분의 1초) 앱이
    ...
    {
		/// 마우스 위치기록(자료형은 Vector2 클래스에서 확인)
