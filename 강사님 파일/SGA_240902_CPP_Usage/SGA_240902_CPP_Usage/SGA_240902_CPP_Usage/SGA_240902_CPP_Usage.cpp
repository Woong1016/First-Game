...
// main에서 작성해도 되고 따로 클래스를 만들어도 됨


//1. 내가 좋아하는 캐릭터의 스탯을 하나의 문자열로 작성해주세요 (이름 제외)
	 //ㄴ캐릭터 스탯은 (쉼표)로 구분해주세요
...
//(옵션) 과제가 끝난 후 c++의 다른 제공된 함수들이나 네임스페이스등을 찾아보시는 것도 좋겠습니다.
...
#include <string.h> // c++ 맞춤 문자열 기능 집합의 신버전
#include <cstring>	// c와 c++에 공통적으로 쓰이는 문자열 기능 집합 약간 구닥다리

//#include "CostumError.h"
//#include "CoustumError2.h"
// 팁 : 오늘은 필요에 따라 위 세 파일을 동시에 쓰지만,
//		사실 동시에 쓰지 않는 편을 추천 (이유 후술)
// 
// 
//우리가 만든 네임 스페이스
...
int main()
{
    ...
    //복습 실행
    //review->Run(); // 복습확인시 주석 풀고 실행

	// 오늘은 조금 쉬운 부분을 다시 보도록 합시다

	// 오늘의 진도 1.: c++의 문자열 관리함수... 의 자주 쓰는 기능 일부


	// 1. 문자열 복붙하기 strcpy_s
	//	실행하면 문자를 새로운 문자로 교체합니다.

	char* word1 = new char[129]; //한글 64까지 버틸 수 있는 포인터 공간 준비

	strcpy_s(word1,9 , "computer"); // 문자열 복붙 (혹은 "덮어씌우기")
									// 메모리 어딘가에 computer 라는 문자를 생성
									// 해당 메모리의 위치를 word1에 할당

									// 매개변수 : word1 <- 복붙한 문자 데이터가 가야 할 곳
									// 9 <- 이오지는 문자의 길이 +1 ("지금 문자열이 끝납니다")
	...
	//3. 문자열 연장하기 strcat_s: cat conCATenation 의 줄임말

	// (일반적으로 생각하는 "복사하기" 혹은 "합치기"의 함수는 이쪽)
	// 위에서 만든 문자를 하나로 합쳐보기

	int lengthOfText1 = strlen(word1); // 문자열1의 길이 내기
	//strcat_s(word1, word2 + lengthOfText2 + 1 , word2);
			//매개변수 = 왼쪽부터 차례로 결과가 담길 표적 데이터(포인터),
			//			해당 데이터에 들어가야 할 문자 개수
			//			합치려는 문자열

	std::cout << word1 << std::endl;

	//4. 문자열을 지정 표시로 구분하기 strtok : tok는 표시 TOKen의 줄임말
	...
	word3 = strtok_s(word1, "r",&wordRemains); // 매개변수 : word1 = 원본 텍스트 (의 포인터)
								//			r = 텍스트를 쪼개는 기준
	...
	//예: 원문은 1,2,3,4,5 라는 텍스트
	// 자를때는 strtok_s(원문 , ",",나머지) 이렇게 사용

	//----------------------------------------------------------------------------------


	// 오늘의 진도 2 : ' 중복되는 함수 ' 와 ' 함수의 영역범위' 에 대해서

		// 배경 : 위에서 문자열 코드를 쓸 떄 _s를 안붙이면 오류가 나는 일부 함수 존재
		//		왜? 같은 함수 정의는 있지만 , 해석이나 취급이 다른 헤더가 있어서
		//		(main함수 위에 너무 많은 문자열 헤더를 썼다!)
	...
	//delete word1; // 지금은 꼭 안이래도 되지만 , "지우기" 실습을 위해 써보기
	//				// 1. 배열 생성은 delete 하고 호환성 떨어짐
	...
	//int a = BrokenClac(10, 3); // 망한 계산 불러오기

	//int b = BrokenClac(10, 3); // 같은 이름의 함수 두번 부르기
	...
	// *그나마 CPP기본 제공 함수 등은 이런 문제가 많이 줄어서 
	// 중복하는 이름이 쓰여도 실행은 되게 만들어 뒀지만
	//그럼에도 불구하고 A함수와 B함수가 서로 달라 한쪽이 무시되거나,
	//A 함수가 다른 헤더에서 "쓰면 안되는 함수로" 로 지정된 탓에
	// A함수를 쓴다고 했다가 오류가 나거나 하는일이 생긴다


	// 이 문제를 해결하는 방법 : 구조체 ㅡ 클ㄹ스와 크게 다르지 않다
	// 데이터를 구조체 , 클래스등으로 묶었던 것처럼
	...
	//int a = Operation(10, 3); // 이따구로는 사용불가능

	//네임스페이스를 사용할때는 마치 클래스 처럼
	//네임스페이스를 먼저 지정을 해주고 뒤에 :: 연산자를 붙여줘서
	//" 어느영역의 함수를 쓸 것인지 " 확실히 해준다음에
	// 거기에 있는 함수를 사용
	...
	int c = CustomNameSpace1::Mult(10, 3);
	...
	// 특정한 네임스페이스를 유난히 자주 쓴다든다
	//특정 네임스페이스의 설정값을 디폴트로 지정하고 싶다면
	//main 위쪽에 using namespace spdlatmvpdltm dlfma dlfkrh cuth
	...
	//main 위에 using na,espace CustomNamespace2를 적은 결과
	...
	//그러므로 using namespace std; 라고 적고 cout를 줄여 써보자
