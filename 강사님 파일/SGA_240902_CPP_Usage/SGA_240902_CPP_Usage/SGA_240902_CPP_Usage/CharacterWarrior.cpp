...
//    // -> 멤버 이니셜라이저가 안 된다!!
//
//	//멤버 이니셜라이저를 사용하는 주체가 자식 클래스가 아니라서.
//	//이니셜라이저의 실행 시점: 클래스가 객체로서 생성되는 바로 그순간
...
// 그렇다면 멤버 이니셜라이저를 상속 관꼐의 자식 클래스에서는 못쓰는가
// 그건 아니다

// 자식 클래스가 부모 클래스를 포함해 멤버 이니셜라이저를 쓰는 방법은
//자식 클래스에서 부모 클래스의 생성자를 같이 써주는것
// 왜? 자식은 부모의 모든 것을 지금 물려받은 상태니까


//부모 생성자를 활용한 진짜 멤버 이니셜라이저 활용 생성자

CharacterWarrior::CharacterWarrior()  :CharacterMain(10,200),rage(100)
{
...
void CharacterWarrior::ShowCharacter()
{
	...
	//(사실 코드 활용 면에서는 그쪽이 더 합리적)


	// 이 떄 부모 클래스의 데이터를 쓰려면 클래스명 :: 문법을 활용하면 된다

	CharacterMain::ShowCharacter(); // 부모 클래스로서 가진 동명의 함수 사용
									// 클래스명:: 문법은 함수 작성에만 쓰는게 아님 
