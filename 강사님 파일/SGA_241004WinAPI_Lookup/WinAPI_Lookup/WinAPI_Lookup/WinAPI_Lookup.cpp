// WinAPI_Lookup.cpp : 애플리케이션에 대한 진입점을 정의합니다.
// 다른 말로, 콘솔 앱의 main 함수에 해당하는 부분을 정의합니다.

#include "framework.h"
#include "WinAPI_Lookup.h"
...
int APIENTRY wWinMain(_In_ HINSTANCE hInstance,         // 지금 만들 윈도우
                     ...
                     _In_ int       nCmdShow)           // 옵션 라인이 (옵션 인수가) 있는가
                                                // 옵션 라인(인수)은 또 뭔가?
                                                ...
                                                // 예 : dir -w /p 에서 -w, 혹은 /p
{
    ...
    // * 윈도우가 맨 처음 만들어지 전부터 실행되어야 할 코드가 있다면 여기서 작성
    //   ......하지만 게임 개발에선 안 쓰인다. (혹은 쓰여야 할 경우 이미 작성된
    //   부분을 가져와서 인용할 것)

    // 전역 문자열을 초기화합니다.
    // -> 전역변수에 있던 문자열(윈도우 이름 등) 수정을 여기서 한다
    //    다른 말로, 윈도우 이름을 정한다
    LoadStringW(hInstance, IDS_APP_TITLE, szTitle, MAX_LOADSTRING);
    LoadStringW(hInstance, IDC_WINAPILOOKUP, szWindowClass, MAX_LOADSTRING);
    MyRegisterClass(hInstance);

    // 애플리케이션 초기화를 수행합니다: = 윈도우 생성
    if (!InitInstance (hInstance, nCmdShow)) // 이게 윈도우 생성 함수 그 자체
                                             // + 더불어서 결과가 성공인지 실패인지 같이 보기
    {
        return FALSE;   // 윈도우가 안 만들어지면 그냥 앱 종료
    }

    // 윈도우가 만들어졌으면 후속 절차 만들기

    // 앱이 OS와 통신하기 위한 준비를 하기
    HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_WINAPILOOKUP));

    MSG msg; // OS로부터 받는 정보가 있으면 이쪽으로 받기

    // OS로부터 정보를 받는다고? - 여러 가지를 받는다
    // 예 : 키보드나 마우스 조작부터, 소리, 버튼이 있으면 버튼 누르기, 클릭과 커서 위치까지
    //      갖가지 정보를 다 받는다

    // 여기까지 애플리케이션 초기화를 수행합니다: => 이후 윈도우 실행 시작

    // 기본 메시지 루프입니다: 위에서 받은 "정보"를 여기서 쓴다
    ...
    // 4. 매시매초 추가 정보를 받으면서 그때그때 들어오는 신호를 처리한다!!
    //    (-> 이게 우리가 보는 앱 실행의 99%)

    // 5. 종료 신호가 오면 최종 결과를 컴퓨터에게 주고 앱을 마친다

    // 이 1~5까지가 윈도우 앱의 구성, 혹은 바로 이 WinMain 함수의 구성
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.hIcon          = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_WINAPILOOKUP));
    wcex.hCursor        = LoadCursor(nullptr, IDC_ARROW);
    wcex.hbrBackground  = (HBRUSH)(COLOR_WINDOW+1); // 기본 배경색 정하기
    wcex.lpszMenuName   = MAKEINTRESOURCEW(IDC_WINAPILOOKUP);
    wcex.lpszClassName  = szWindowClass;
    wcex.hIconSm        = LoadIcon(wcex.hInstance, MAKEINTRESOURCE(IDI_SMALL));
    // 여기에 추가 정보가 있으면 추가 변수를 설정해줄 수도 있다 (예 : 제목, 구체적 크기...)
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow) // 반환형이 BOOL인 것 확인
{
   ...
   {
      return FALSE; // 안 됐으면 실패로 반환 (그리고 WinMain에서 이 값을 받아 강종)
   }
   ...
   return TRUE;     // 성공했음을 반환하고 윈도우 생성 종료
}
...
// 
//  WM_COMMAND  - 애플리케이션 메뉴를 처리합니다. (처음 시작할 때, 혹은 코드가 직접 지시할 때)
//  WM_PAINT    - 주 창을 그립니다. (출력 신호가 왔을 때 = 실질적인 '매시매초' 중 하나)
//                = WM_PAINT가 들어올 때마다 화면을 그려주면 일종의 잔상 효과로
