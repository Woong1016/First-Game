...
struct Monster          // 괴물
{
    ...
    //  해당 존재는 아래와 같은 세부 정보를 갖는" 걸로 정한다
...
// 지난 시간에 진행했듯, 위 '괴물'의 구조체를 자료형인 것처럼 사용하면
// 앱 안에서 실제 괴물 개체를 하나하나 구현할 수 있다
// 예 : int a = 1, float f = 10.0f, '괴물' M = 어떤 괴물, '괴물' M2 = 저떤 괴물

// 그리고 개별 괴물에게 붙여줄 이름을 따로 작성한 후 (현재는 문자열의 수정이 안 돼서)
...
{
    // * 특수한 상황! : 여기의 데이터는 나중에
    // 바뀔 필요도 없고 (중요한 기초 데이터),
    // 바꿀 수도 없으므로 (배열 이름 = 상수 포인터)
    // 여기서 그냥 변수 값을 할당합니다.
...
enum CHECKANSWER    // 답변에 대한 정오를 확인하는 열거형
{
    ...
    // 그런데, 이 상황에서 RIGHT라는 단어를 덮어놓고 못 쓰면 안 되는데?
    // -> 다른 경우에도 이렇게 동음이의어 등으로 꼭 필요해서
    //    철자가 겹치는 다른 단어를 다른 열거형으로 만들어야 할 경우는
    //    ...그럼 어쩌지?

    // 이론적으로 매우 간단한 답이 있다
    // '이 RIGHT와 저 RIGHT는 완전히 다르다'라는 걸 정해주면 된다.

    // 그런데 어떻게? ..... 여기서 C++ 이전의 프로그래밍은
    // 이 문제를 더는 해결 못 한다. 그러므로...

    // 이제부터는 C++로부터 시작한 고유 기능을 하나씩 써야 할 차례
    // -> 자세한 건 main()에서 다시 설명
};
