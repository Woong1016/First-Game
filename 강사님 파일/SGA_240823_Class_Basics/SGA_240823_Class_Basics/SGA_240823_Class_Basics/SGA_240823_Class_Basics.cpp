...
// 주말에 클래스에 익숙해지기 위한 과제

// * 사칙연산 계산기를 이번에는 클래스로 만들기
...
// 4. 그리고 두 숫자의 값을 main()에서 할당해주세요.
//    (접근 지정자를 쓰든, 다른 함수를 작성하든, 자유)

// 5. main()에서, 계산기 객체의, 사칙연산 함수를 사용해주세요.

// 6. 사칙연산을 수행한 결과를 모니터에 출력해주세요.

#include <iostream>
#include "SampleOfThePrevious.h"

#include "MyClass.h" // 클래스 파일 포함 (설명은 아래 본문)
...
int main()
{
    ...
    // 2. 문자열을 수정 못 하는 거 너무 귀찮다 (c 및 c++ 기본 기능의 한계)

    // 3. 구조체는 왜 상수를 못 쓰나? 상수 좀 쓰게 해 달라!

    // 4. 그래서 상수 포인터를 쓰는 건 알겠는데... 그 때 사용한
    //    원시 데이터가 결국에는 자꾸 눈에 밟히지 않는가?
    //    '쓰지 말라고 했다 한들' Monster의 _var0000을 누가 불러내서 썼다면
    //    이거 어떻게 막을 건가?

    // 다른 말로, "데이터를 아예 눈에도 안 보이게 만들 수는 없을까?" = 보안문제

    // 5. 구조체가 대상의 개념화인 건 알겠는데... 그래서 왜 데이터만 있느냐?

    // -> 개념의 대상이면 이거 행동도 할 수 있어야 되는 거 아니냐?
    
    // 다른 말로, 이 안에 함수도 있으면 좋은 거 아니냐?
    ...
    // 위 서술을 용어로 줄여보면 :
    // 1. 독립, 2. 은닉, 3. 캡슐화(은폐, 간편), 4. 주체화

    // 이렇게 4가지를 전부 이뤄낼 수 있는 새로운 개념이 필요해진다

    // 다시 말해, 데이터로서만 아니라, 데이터도 당연히 있고
    // 그 안에 실제 행동 기능 (=함수) 까지 전부 들어있는
    // 그런 가상의 개념 범위가 있어야 된다

    // c++에, 그리고 c++에서부터 이 기능을 구현하는 방법이 있다
    ...
    //   '함수가 들어간다'는 특성상(= 적다 보면 길어지기 쉽다 = 읽기 귀찮아진다)
    //   대체로 별도 파일에 작성하는 경우가 많다. -> 별도 파일 작성 추천

    // * 파일>새로 만들기>파일, 혹은 솔루션 탐색기 우클릭>추가>클래스

    //  ...을 한 다음, 클래스 선택 > 클래스 이름을 지정

    //--------------------------------------------------------

    // 클래스 작성을 MyClass.h에서 진행하고 여기서 선언 및 호출

    MyClass myClass; // '변수' 호출

    //즉 구조체처럼 클래스도 그 자체로는 아직 진짜 데이터가 아니다
    // -> 일종의 설계도, 혹은 '자료형 비슷한 뭔가'이다

    // 실제로 이걸 사용하려면 위처럼 '클래스 자료형의 변수'를 만들어야 한다
    // 역시 구조체와 마찬가지로, 이 클래스를 통해서 만든 진짜 데이터는

    // 이제 '변수'가 아닌 '인스턴스'로 부른다.
    // 위 선언에서 MyClass는 클래스, myClass가 인스턴스.
    
    // (특히 '기능' 즉 함수를 포함하기 때문에 변수라는 말은 부적절하기도)
    ...
    //std::cout << myClass.a << std::endl; // 구조체처럼 안의 변수 출력하기
    //std::cout << myClass.b << std::endl; // ...했는데 안 된다? 오류가 난다?

    // 이것이 클래스의 첫 번째 장점.
    // -> 만든 사람이 허락하지 않았으면, 클래스 안에서 만들었어도
    //    밖에서 호출하는 입장에서는 변수 a, b를 사용할 수가 없다
    //    그뿐만 아니라 애초에 그런 변수가 있는 줄도 모른다 (안 보인다)
    //    이것이 은닉, 은폐(캡슐화)의 구현

    // 따라서 클래스에서 작성된 데이터든, 함수든, 밖에서 사용하려면
    // 작성하는 쪽에서 쓸 수 있게 허락을 해줘야 한다.

    // 그렇게 허락한다는 지시를 작성한 코드를 '접근자' 혹은 '(접근) 지정자'
    // 라고 한다. (실제 작성은 클래스에서)

    // MyClass에서 만든 public 접근 지정자 데이터 사용

    std::cout << myClass.x << std::endl; // x 출력 가능
    std::cout << myClass.y << std::endl; // y도

    // 클래스에 익숙해지는 동안에는 public으로 그냥 다 작성해도 무관

    // private 접근 지정자 시험도 수행
    // std::cout << myClass.f << std::endl; // 오류!
    ...
    // 왜냐면 저 a와 b를 출력하는 기능은 PrintAnB 함수 안에 있는데,
    // 그 함수를 실행한 주체는 main() 앱이 아니라 myClass니까.
    // myClass 입장에서는 a와 b는 자기 안의 내부 데이터인 것

    // -> 그 내부 데이터를 myClass가 고치고 출력하는 것도 되는가?
    // -> 함수 작성 후 시험해보기

    myClass.SetNumberToAnB(50000, 7777); // 새로운 숫자를 부여

    std::cout << myClass.AddAnB() << std::endl; // 새로운 두 숫자를 더하고 출력
    ...
    // 3. 만든 클래스는 구조체와 마찬가지로 인스턴스로 만들어 쓴다
    //    클래스와 클래스의 인스턴스를 합쳐 객체라고 부른다

    // 4. 클래스 안에 작성된 데이터는 기본적으로 밖에서 볼 수 없다
