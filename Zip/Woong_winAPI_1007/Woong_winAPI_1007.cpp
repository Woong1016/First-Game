// Woong_winAPI_1007.cpp : 애플리케이션에 대한 진입점을 정의합니다.
//


// 과제 : 브러쉬 예습하기

//오늘의 과제 : winAPI로 그림그려보기

//글자 , 자유직선 , 직사각형, 타원 등을 이용해

// 무엇이든 좋습니다. 여러분이 좋아하는 그림을 하나 그려보세요

// 그리고 원하면 InvalidateRect 함수는 과제에서 빼셔도 됩니다.
...
// * 나중에는 이 둘을 묶어서 적절한 구조체를 만들 예정.. 이지만 시간상 기본변수 2개만..
                     ...
                     _In_ int       nCmdShow)
{
    ...
    LoadStringW(hInstance, IDC_WOONGWINAPI1007, szWindowClass, MAX_LOADSTRING);
    ...
    HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_WOONGWINAPI1007));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.hIcon          = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_WOONGWINAPI1007));
    wcex.hCursor        = LoadCursor(nullptr, IDC_ARROW);
    wcex.hbrBackground  = (HBRUSH)(COLOR_WINDOW+1);
    wcex.lpszMenuName   = MAKEINTRESOURCEW(IDC_WOONGWINAPI1007);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        ...
        break;
    case WM_PAINT: // 화면표시 메시지, OS가 자동으로 호출 수행한다
        {
            ...
            HDC hdc = BeginPaint(hWnd, &ps); // HDC : 출력 매체 관리 데이터
											// handler of 'Devoce Context'
											// 출력 매체 혹은 " 디바이스 컨텍스트"
											// = 다양한 출력도구를 OS가 관리하기 위해
											// 하나의 통칭으로 편의상 지칭하는 것
										// 우리한테는 "모니터" 쯤으로이해하면 된다.

			//어쩄든, 이렇게 출력 도구에 대한 데이터를 미리 준비해서
			// " 그릭 준비(Begin paint) 절차를 시작한다 (이때 인수가 hwnd , &ps)
			// hwnd: 이화면을 그려낼 영역 = 앱이 실행된 윈도우
			
			// 그리고 지금 이줄부터 코드를 작성해서 , 화면에 영향을 끼치면
			...
			//		true / flase : 기존 화면을 완전히 지울 것인지 말 것인지.


			

			//---------------------------------------------------------------
			//std::cout < 이 코드는 winAPI 환경에선 더이상 안쓴다.
			//printf_s < 이놈도 안씀

			//winAPI에서 문자 출력수단 " TestOutW.. 라는 함수
			...
			TextOut(hdc, 300, 300, L"박진웅은 배고프다!", 10);
			// 마지막 글자 수 인수가 문자 개수와 안맞으면 빈 칸만큼 이상한 기호 출현
			/// 또한 윈도우 문자 설정에따라 , 한글을 적어도 1글자로 취급되거나
			// 2바이트 이상 문자를 추렭할 때 크기가 늘어나서 출력되거나 둘 중 하나
			...
			// 각각 출력위치 , 가로지정 위치, 세로 지정위치 , 추가요청사항

			//두번째 : "직선 그리기"

			LineTo(hdc, 600, 400); // 초점을 옮기면서 , 궤적을 그리는 코드
			//따라서 MoveToEx 함수로 컴퓨터가 먼저 500, 50 위치에 초점을 옮기고 
			//(이때는 아무것도 안그려짐)
			...
			//=> 윈도우에서 실행 중 머우스 커서 위치에 그림 다시 그리기

			// *준비물 : 전역변수 (소수)2개 , 이 전역변수에 마우스 위치 담기
			// 0-> 전역변수 자리에 만든 후 , 값을 할당하고 돌아온다.

			// 위에서 만든 mouseX , mouseY 를 사용해서 도형이 그려질 위치 지정
			
			Rectangle(hdc, mouseX - 100, mouseY - 100, mouseX + 100, mouseY + 100);
			...
			// 전역변수는 실제로 잘 동작했다
			// 그리고 0,0 위치는 왼쪽 위다 (원점)_
			// 그런데 사각형은 마우스 커서를 따라가지 않는다!!

			//이상의 사실을 확인할 수 있다

			// 이유 : 그려진 화면에 대한지시가 코드에 없었기 때문 (정보폐기만 있음)

			//따라서 , 화면에서 정보가 바뀜에 대한 화면 갱신 코드를 추가해줘야 한다

			//그 함수가 InvalidateRect() <= 여기 말고 위에

			//--------------------------------------------------------------------------
			//그리기가 다 끝난 후 , End Paint 절차를 통해서 화면 그리기 작업에 동원한
			//데이터의 메모리 점유를 해제해준다 (안하면 작업 수행 시마다 메모리 점유 up)
			// 다시 그리기가 시작되어야 하면 위에 있는 Begin Paint 부터 재시작...!1!!!!!!!!
		...
		case WM_MOUSEMOVE: // OS에서 감지한 메시지 내용이 마우스 이동에 대한 것일 때
			...
			//메시지에서 같이 들어온 매개변수 두개가 마우스 가로 , 세로 위치
			// => 그 데이터를 준비해둔 전역변수에 넣으면 된다.

			mouseX = (float)LOWORD(lParam); // 매개변수 중 'lParam' 부분에서 먼저 들어온 부분을 뜻하는 LOWORD 라는
			mouseY = (float)HIWORD(lParam); //

			//lParam : 마우스 정보가 들어가 있는 큰 매개변수 단위 , 긴 패러미터.
			//LOWORD : 긴 패러미터 중에서 먼저 들어온 정보
			//HIWORD : "	뒤에 들어온 정보
			//* 두 정보 값이 더블(배정밀도)이므로 형변환을 직접해서 오차 감소 의도
			break;
