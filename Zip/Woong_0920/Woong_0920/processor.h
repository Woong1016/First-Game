...
//				이어지는파일 이름과 합쳐서 해당 파일을 코드 내용에 포함시킨다.
//				"사전 로딩" 정도로 이해해도 좋다.


// * include 대상 활용 팁, <> ""의 차이
//	<>는 c++의 내부 기능이란 뜻. c++이 설치된 컴퓨터라면 해당 기능은 있는 걸로 친다.
// 특히 윈도우의 경우, 윈도우 폴더 안에 지정하는이름의 파일이 실제로 있다.
//(대부분 system32 , system62)

// ""sms c++의 기본 내부 기능에는 없었고, 나중에 사용자 (프로그래머)가 
//	자기 폴더에 (그러니까 지금 여기에) 직접 추가한 파일이라는 뜻.
// 만들었거나, 다른 곳에서 복사해서 폴더에 붙여넣기 했거나 . 

// -> 그래서 우리가 만든 헤더는 ""로 , string, vector 등을 포함할 떄는
// <string> <vector> 등으로 포함했던 것

#include <stdlib.h>// c++이면 어련히 갖고 있을 것 같은 그 기능들
#include "헤더.h" // 우리가 만들어서 지금 이 폴더에만 존재하는 파일


// - 그래서 이 코드를 쓸 때는 약간의 주의사항이 포함된다.
//<>로 기능을 넣을 때는 다른 컴퓨터가 같은 버전의 C++을 갖고 있는지 확인 필요
// ""로 기능을 넣을 때는 이 폴더에 있는 파일이 100% 온전하게 공유되도록 조치필요


// *본론 *

// 3. define : 흔히 매크로라고 부르는, 코드 텍스트 변환지시

// => 이어지는 단어 둘을 연결시켜서 , 이후 첫 단어를 입력하면 , 두 번째 단어로 바꾼다.



#define ARRAY_SIZE 10 // ARRAY_SIZE라는 단어를 10 이라는 단어로 바꾼다
					// 주의 : 코드나 데이터를 바꾸는게 아니다.
					//			진짜 텍스트를 바꾸는 것

// -> 나중에 코드 사용처에서 첫 단어(여기서는 ARRAY_SIZE)를 입력하면
//		실제로 컴퓨터가 컴파일을 할 떄는 이 텍스트가 10인 것으로 받아들이게	된다.
...
#define TEST_STRING "안녕하세요 반가워요 잘있어요 다시만나요"
...
// 전처리기 의 매크로 명령어에는 한 가지 기능이 더 있다....

//바로 "해당 단어가 선언된 적이 있다는 사실 그자체"

#define IAMHERE // 뒤에 아무것도없음 따라서 변환불가

//--> 아무 것도 없어서 아무데도 쓸모가 없는 거 아니냐 싶지만 , 그렇지만도 않다
// 바로 이 단어가 존재한다는 사실 자체가 중요하다.

//이 코드를 내가 적음으로 인해 , 지금 이 파일은 컴파일 중에

// 이후 IAMHERE라는 단어를 또 보게 되면 반드시 그에 반응할 것
...
//일반 조건문과 추가로 다른점은 전 처리 단계에서 실행하므로
...
// -> 메인함수에서 사용
//노는게 제일 좋아 친구들 모여라 언제나 즐거워 개구쟁이 뽀로로 숲속마을 꼬마 팽귄 나가신다. 언제나 즐거워 오늘은 또 무슨일이 생.길. 까

// 뽀로로를 불러봐요 뽀롱뽀롱 뽀롱뽀로롱 뽀로로뽀ㅗㅇㄹ뽀로ㅗㅇ오ㅃ로ㅗ뽀로로
