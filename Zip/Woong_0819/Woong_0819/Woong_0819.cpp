...
int main()
{
	...
	*aPtrCon = bVar; // 또 이건 왜 됨?
	...
	int const nIC = 100000; // 이것도 사실은 상수
							// 그런데 왜 안썼을까?

	// const의 의미가 증갑 연산자와 비슷하게 작동하기 때문
	// const : 따라오는 데이터를 컴퓨터가 다른 사람이 (미래의 나 포함)
	//		함부로 바꾸지 못하게 만드는 기능을 한다


	// const + int : "상수가 있고 , 그 상수는 int 다 :


	// int + const : "int 가 있고 이 int 는 상수다" 라는 의미
	...
	// 상수 포인터의 원문  : pointer to constant  ' 중요한건 사이에 있는 to
	// 우리말의 의미는 '상수로 *가는*포인터 라는 뜻

	// 쉽게 풀면 "원래 상수였든 아니든, 지금부터는 상수처럼 쓰겠다" 라는 것

	//무엇을? 이 데이터를. 포인터가 지시하고 있는 대상을.

	// 어떤 사람은 그래서 이 의미를 상수로 만드는 포인터 라고도 부른다

	// 그래서 상수 포인터는 다음 용례가 성립한다

	//*conPtr = 7; // 포인터가 가리키는 메모리 주소의 데이터는 더이상 못 고친다.
					// 마치 상수인 것처럼 취급한다.
	...
	// 역참조 대상인 실제 데이터는  못고치는 대신 , 포인터의 지칭 대상은 수정가능

	//conPtr = &bVar; // 상수 포인터가 지칭했던 대상을 n 에서 bVar로 바꾸는 건 된다.
	...
	// 결론 : 상수 포인터는 참조된 데이터를 상수로 만드는 포인터

	// 값은 못바꿈 값을 바꾸려는 대상을 바꿀 수 는 있음


	// 포인터 상수

	// 위에서 말한 , 다른쪽 상수 포인터의 반대라고 보면 ok

	//정확히 말하면 이쪽의 원문은 constant pionter

	//우리말로는 포인터의 모양을 한 상수 라는 뜻


	// 기본형태 : (자료형)* const (포인터가 , 지금부터, 상수다)
	...
	*ptrCon = 7788;		// 역참조의 대상은 수정 가능
	...
	ptrCon = &bVar; // 포인터로서 참조해야 할 대상은 이제 수정 불가능

	// 따라서 내부의 데이터는 고칠 수 있지만 , 참조해야할 대상은 무조건
	...
	const char* wordCP = &word; // 외부에는 이렇게 지칭을 시켜준다		
									// 그러면 외부에서는 wordCP를 못고친다

	*wordCP = "헬로 월드!"; // 이거 안됨

	// c++의 경우 char 특수한 취급이어서 포인터 + 배열 관점에서도 수정이 안된다
	// 다만 나중에 문자열 기능을 접하면 , 그때도 이 상수 포인터가 적용된다.

	// 더불어서 상수 포인터를 쓰는  다른 이유도 잇다

	// 특수한 경우지만 .... 쓸 때는 상수로 되는 변수가 있는데

	// 만들때는 미리 값을 지정할 수 없는 경우가 있다.

	// -> 상수는 이게 허용이 안된다. (선언과 동시에 값이 있어야만 한다.)

	const int id; // 이거 안됨

	// 그래서 일단 변수부터 만들고 나중에 값을 주되, 그래도 상수인 것처럼 쓰려고 할때
	// 그때 상수 포인터를 활용할 수 있다.

	int var000; // 이상한 이름을 가진 실제 데이터를 숨기고

	const int* ID = &var000;// 외부에서는 이 데이터만 보여준다

	// 위의 id 처럼 미리 id 데이터를 자리만 만들어두고 , 값은 나중에 주고,
	// 이 데이터를 일종의 상수로 (고칠 수 없게 ) 만들려고 할 떄 상수 포인터를 활용한다

	//물론 var000처럼 본래 데이터가 들통나면위험은 하지만...

	// 아예 무방비한 것보다는 조금 낫다.



	// 그러면 포인터 상수는 언제 왜 쓰는가?
	...
	// 그 위치 자체는 절대 바뀌면 안되는 경우가 있다.

	// 가장 대표적인 것 : 온라인 게임의 플레이어 데이터

	// 이렇게 참조의 일관성을 유지해야 할 떄, 포인터 상수를 사용한다.
	...
	const int* const xConPtrCon = &aVar; // const +wkfygud +* +const

	//용례 라기보다 그냥 안되는 것을 확인

	xConPtrCon = &bVar; // 참조 수정 못함 (포인터 변수가 그자체로 상수)
	*xConPtrCon = 1000000; // 역참조 대상 수정 못함 (실제 데이터도 상수처럼

	aVar = 999; // 물론 원본은 아직 상수 아님

	// 포인터 변수에 극한의 구속을 주고 싶을 때 (참조대상도 실질 데이터도)
	// 그때  이렇게
