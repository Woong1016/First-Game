...
int main()
{
	...
	// 모든 학생은 (문명사회에서) 지하철 탈줄 알고 투표할 자격이 있다.
	// 또한 모든 시민은 밥 먹고 움직일 줄 알 것이다. 

	//= 학생이라는 말 자체가 시민을 전제한 것. 시민은 곧 인간의 일부분
	// 다른말로 " 인간 안에 시민이, 시민의 한 부분에 학생이라는 정체성이" 존재

	//이렇게 표현 가능한 대상의 소속의 계층 순서를 따라

	// 클래스를 코드로 구현해 보고자 한다다다다.
	...
	Human* human = new Human(); // 생성자를 안만들었는데, 괜찮은가?
								// -> 괜찮다 , 왜냐면 모든 클래스는 
								//  자동으로 숨겨진 기본 생성작가 있어서
	...
	citizen->birthday = 20011016; // 생년월일 할당
	citizen->Subway();				//지하철 타기


	// 모든 시민은 (지금은) 곧 인간이므로 , 인간의 행동도 가능...?

	citizen->age = 28; // 부모 클래스 변수 호출 가능 예림님이 저 28살이라 해서 24살인데 28살로 적었어요..
	citizen->Eat();

	//human->Workout(); // 보호 데이터 부모 클래스에서 사용 불가
	//citizen -> Workout(); // 보호 데이터 자식 클래스에서 사용 불가

	citizen->WorkoutAsHuman(); // 자식 클래스의 public 
	...
	student->birthday = 20011016; // 부모 데이터도 사용
	student->age = 28; // 부모의 부모 데이터도 (public이라면 ) 모두 사용
	...
	student->GoDoctorAsCitizen();  



	// 이렇게 자식 클래스를 써보는 것으로 확인 가능한 것

	// 자식 클래스를 만들고 , 부모 클래스를 public 으로 상속하고

	// protected 와 public 을 잘 써주면
	...
	Woker* woker = new Woker(); // 또 다른 시민의 자식 객체

	woker->Work();// 시민 함수 호출 가능
	woker->Eat(); // 부모는 물론 부모의 부모 함수도 다 호출 가능
	...
	//2. 자식 클래스의 부모 함수 갱신 (혹은 덮어씌우기, override)


	// -> 노동자 클래스에서 확인


	student->GoDoctorAsCitizen(); // 학생 안에 있는 , 시민의 " 병원가기" 실행
	woker->GoDoctor(); // 노동자에서 public 으로 고쳐진 , 같은 철자 함수 실행 

	// 위 함수는 따지고 보면 똑같은 GoDoctor() 함수를 실행할 것
	...
	// 2. 덮어씌이는 과정에서 접근 지정자 역시 자식 클래스의 것으로 갱신된다. 
	...
	woker->Drive(); // 시민 클래스에 있던 Drive 함수를 노동자 클래스에서
					// 오버라이드 한 것
