...
// 주말 과제: 포켓몬스터 엔트리 ,혹은 인벤토리 만들어보기

//1. 빈 벡터 하나와 , "항목으로 채워진 벡터 하나를 만들어주세요"

//(여기서 항목은 여러분들이 생각하는 목록의 내용을 뜻합니다. 포켓몬 , 아이템등)
//2. 키보드 입력을 받아서 항목 벡터에서 원하는 항목을 선택할 수 있게 해주세여

// 3. 선택한 항목은 빈 벡터에 차례대로 들어가야합니다.
// 이를 엔트리, 혹은 인벤토리로 지칭합니다.

// 4. 엔트리 및 인벤토리는 최대 6개에서 8개까지 추가할 수 있게 해주세요

//5. 엔트리 및 인벤토리에서 역시 키보드 입력을 받아서
// 증긴에 항목을 지워 없앨 수 있게 해주세요

// 따라서 인벤토리 추가 및 삭제도 선택할 수 있어야 합니다.
...
// 옵션 2. 엔트리 및 인벤토리에 변동이 생길경우, 해당 항목을
...
int main()
{
	...
	vec.insert(it + 4,9999); // 지정한 순번에 새로운 원소를 추가
								// 원래 있던 원소는 뒤로 밀려난다
	//							// 중간에 숫자를 끼워넣는 모양새
