...
{
	myDrink->Drink(); // 음료수 만들기
	std::cout << "지금 실행된 대상이 음료수 클래스임을 확인 합니다." << std::endl;

}

// 함수 오버로드를ㄹ 사용할 경우 예시
void OrderAndDrink(Coke* myDrink) {} // 음료수 클래스를 매개변수로
void OrderAndDrink(Beer* myDrink) {} // 음료수 클래스를 매개변수로
...
// 음료수 클래스가 있어서 써본다고 치자
	...
	// 원래코드
	//Baverage* baverage = new Baverage(1000, "아무거나");

	//* 인터페이스 적용후 코드 (아래 및 Baverage 해설 참조)
	...
	// 그럼 음료수 클래스를 상속받는 자식 클래스를 만들면 될텐데
	...
	baverage->Drink(); // 아무거나 를 마십니다 호출됨

	//그리고 이제 다른 음료수도 만들어서 자기만의 함수를 사용한다면
	// 새 객체 만들기( 자식 클래스에서)

	Coke* coke = new Coke(2000, "마시자 코카콜랄라라랄");
	Beer* beer = new Beer(2500, "써머스비");
	...
	// 
	// 예시1. 자식 클래스가 -> 함수를 사용하는 것이 아니라
	//		함수를 불러야 할곳에서 -> 자식 클래스의 객체를 쓸 경우?
	// 수업 들으세요.
	...
	//히지만 자식 클래스가 지금보다 더 많아진다면?
	//혹은 클래스를 매개변수로 받아야 할 함수가 지금보다 더 길어진다면?
	// 그때마다 오버로드를 하나하나 작성할 수는 없다.

	//예시 2. 3번째 자식 클래스를 만약 만들었는데 , 함수에 Drink가 없다면??
	//		혹은 만들었는데 이름이 다르다면?
	//			=> 의외로 협업 체제에서 자주 일어나는 일
	// (* 옆사람, 그리고 "미래의 나" 만큼 내 말 안듣는 사람도 없다.)

	// 기존의 코딩은 이 두가지를 해결하거나 강제할 방법이 없다
	...
	OrderAndDrink(interfaceInstance); // Beer로 만들었지만 , Baverage는 가능
	...
	// 이렇게 가상함수를 사용한 규칙 강제 (인터페이스) 방법을 쓰면
	// 자식 클래스를 부모의 규칙에 맞추어서 무조건 작성하게 ㄱ강제할 수도 있고
	// 자식 클래스가 아무리 늘어나도 , 어떤 자식 클래스가 와도
	//부모 클래스 하나만으로 대응할 수가 있어서 편하당께

	// 단, 부모 클래스의 함수중에 내용이 이쓴ㄴ 쪽을 슬 경우//...
	...
	// 이런 코딩 방법을 업캐스팅, 다운 캐스팅이라고도 하는데... (관심있는 분 검색)
	// 현실에서 중요하게 쓰는 용어는 아니다
