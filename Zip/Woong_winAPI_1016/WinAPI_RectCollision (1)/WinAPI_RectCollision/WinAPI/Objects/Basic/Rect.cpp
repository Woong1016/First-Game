...
bool Rect::IsCollision(const Circle* circle) const
{
	...
	if (circle->pos.x > Left() && circle->pos.x<Right() || // 원의 중심이 사각
		circle->pos.y>Top() && circle->pos.y < Bottom())	// 원의 중심이 사각
	{
		//두 조건중 하나만 충족해도 십자 범위 안으로 들어온 ㄱ것

		// 이때: 사각형의 가로 세로 크기(의 반)와 원의 반지름 비교

		Vector2 size(circle->GetRadius() * 2, circle->GetRadius() * 2);
		//x2를 할 경우 원을 감싸는 큰 사각형을 만들어서 통짜 계산 가능
		// 통짜 계산?: 원의 5방향 어느쪽에 상관ㅇ ㅓㅄ이 적용가능한 반지름 범위 생성

		//원의 가로세로 크기를 나타내는 범위 생성

		Rect tempSize(circle->pos, size); // 원의 위치 , 원의 크기

		// 이 사각형과 만들어진 범위가 충돌하는지 본다
		...
		else return false; // 충돌을 안 했으면 그냥끝
	...
	// 원이 십자 범위였으면 위 코드에서 충돌여부가 완전히 계산 후 , 함수종료
	...
	for (Vector2 point : edge) // 위쪽 반복문과 기능이 같다
	{
		if (circle->IsCollision(point)) return true; //
	}

	//for (자료형 name : array )문법
	//=>array 라는 배열을 구성하는 원소를 임시로 name 이라고 명명한 후
	// 배열 속에 있는 모든 원소 즉 name을 모두 돌아가면서 검사한다.
	//	배열의 크기가 정확히 뭔지 몰라도 알아서 배열을 반복한다.

	//이것을 for each 코드라고 한다
	//-> 속도가 느린편, 배열에 어떤 원소가 있는지 잘효형을 정확히 알아야 한다.
	// 그래서 콘솔 단계나 엔진 단계에서는 그렇게 선호되지는 않고
	// 다만 윈 API나 DX 단계에서는 어느정도 쓰이므로 이 참에 경험해보자
